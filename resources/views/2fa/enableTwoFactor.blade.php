@extends('layouts.app')

@section('content')
<div class="container spark-screen">
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                <div class="panel-heading">2FA Secret Key</div>

                <div class="panel-body">
                    Open up your
                    <a href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=en" target="_blank">2FA mobile app</a> and scan the following QR barcode:
                    <br />
                    <img alt="Image of QR barcode" src="{{ $image }}" />

                    <br />
                    If your <a href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=en" target="_blank">2FA mobile app</a>  does not support QR barcodes,
                    enter in the following number: <code>{{ $secret }}</code>
                    <br /><br />
                    {{--  <a href="{{ url('/courier') }}" class="btn btn-primary">Go To application</a>  --}}


                    <form action="/logoutOther" method="get">
                        @csrf
                        <button type="submit" class="btn btn-primary">Logout other devices</button>
                    </form>

                </div>
            </div>
        </div>
    </div>
</div>
@endsection
